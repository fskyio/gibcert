...
//
// Modified from the original github.com/mholt/acmez v3.1.6 tlsalpn01.go when
// vendored into gibcert: moved from package acmez into package acme, and the
// golang.org/x/net/idna dependency was removed (DNS identifier values are used
// verbatim, consistent with the rest of gibcert's issuance path). See NOTICE.
...
func TLSALPN01ChallengeCert(challenge Challenge) (*tls.Certificate, error) {
	dnsNames := []string{}
	ipAddresses := []net.IP{}
	...
	case "dns":
		// Certificates must encode their SANs as ASCII. gibcert uses identifier
		// values verbatim throughout issuance (the CSR SANs likewise), so any
		// internationalized name is expected to already be in punycode form;
		// keeping that consistent here avoids pulling in an IDNA dependency.
		dnsNames = append(dnsNames, challenge.Identifier.Value)
	case "ip":
